...
def base(event_type: str, session_id: str, *, payload: Optional[dict] = None,
         project_id: Optional[str] = None, turn_id: Optional[str] = None,
         channel: str = "cli", event_id: Optional[str] = None,
         parent_event_id: Optional[str] = None) -> dict:
    ev = {
        "source": SOURCE,
...
def structured_directive(session_id: str, event_type: str, text: str, *,
                         source_event_id: str, project_id=None, channel="cli") -> dict:
    normalized = " ".join(text.split())
    key = f"{event_type.lower()}:{hashlib.sha256(normalized.casefold().encode()).hexdigest()[:16]}"
    eid = _event_id(event_type, session_id, source_event_id, normalized.casefold())
    payload = {
        "text": text, "item_key": key, "status": "OPEN" if event_type == "TASK_UPDATE" else "CONFIRMED",
        "confidence": 1.0, "extractor": "EXPLICIT", "source_event_ids": [source_event_id],
    }
